...
<#
.SYNOPSIS
    Publishes the Uk.Legislation NuGet package after running validation checks.
...
Write-Host "  UK Legislation .NET Library Publisher" -ForegroundColor Magenta
Write-Host "========================================`n" -ForegroundColor Magenta
...
# Configuration
$projectPath = "Uk.Legislation\Uk.Legislation.csproj"
$testProjectPath = "Uk.Legislation.Test\Uk.Legislation.Test.csproj"
...
$packageFiles = Get-ChildItem -Path ./nupkg -Filter "Uk.Legislation.*.nupkg" | Where-Object { $_.Name -notlike "*.symbols.nupkg" }
...
if (-not $version) {
    if ($packageFile.Name -match 'Uk\.Legislation\.(\d+\.\d+\.\d+.*?)\.nupkg') {
        $version = $matches[1]
...
if (-not $DryRun) {
    ...
    Write-Host ""
    Write-Host "  Package: Uk.Legislation" -ForegroundColor White
    ...
    Write-Info "View at: https://www.nuget.org/packages/Uk.Legislation/$version"
}
else {
...
if (-not $DryRun) {
    Write-Success "Package published: Uk.Legislation v$version"
    Write-Info "It may take a few minutes to appear in NuGet search results."
    Write-Host ""
    Write-Info "Next steps:"
    Write-Host "  1. Create GitHub release: https://github.com/panoramicdata/Uk.Legislation/releases/new" -ForegroundColor White
